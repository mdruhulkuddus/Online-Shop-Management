<?php
	...
	if(isset($_POST["add"]))
	{
		if(!empty($area))
			...
			{
				mysql_query("insert into aboutus(`text_area`,`id`) values('$area','1')");
	...
	if(isset($_POST["edit"]))
	{
		if(!empty($area))
			...
			{
				mysql_query("replace into aboutus (`text_area`,`id`) values('$area','1')");
				if(mysql_affected_rows()>0)
				{
				print "Data ADD successsfully";
				}
...
<html>
	...
	<form name="about" method="post" >
 				<table height="424" width="506" cellpadding="0" cellspacing="0" align="center"  >
					<tr>
						<td colspan="3" height="82" align="center" bgcolor="#5481FC"><font class="title" ><font color="FF0000">A</font>bout<font color="FF0000"> U</font>s</font></td>
					</tr>
					...
					$selct=mysql_query("select * from aboutus WHERE id='1'");
